...
class QWeb(models.AbstractModel):
    ...
    def _generate_asset_links_cache(
        ...
        res = super()._generate_asset_links_cache(
            bundle, css=css, js=js, assets_params=assets_params, rtl=rtl, autoprefix=autoprefix,
        )
        if bundle == "web_company_color.company_color_assets":
            asset = AssetsBundleCompanyColor(bundle, [], env=self.env, css=True, js=True)
            res += [asset.get_company_color_asset_node()]
        return res

    def _generate_asset_links(
        self, bundle, css=True, js=True, debug_assets=False, assets_params=None, rtl=False, autoprefix=False,
    ):
        res = super()._generate_asset_links(
            bundle, css=css, js=js, debug_assets=debug_assets, assets_params=assets_params, rtl=rtl, autoprefix=autoprefix,
        )
        if bundle == "web_company_color.company_color_assets":
            asset = AssetsBundleCompanyColor(bundle, [], env=self.env, css=True, js=True)
            res += [asset.get_company_color_asset_node()]
    ...
    def _get_asset_nodes(
        self, bundle, css=True, js=True, debug=False, defer_load=False, lazy_load=False, media=None, autoprefix=False,
    ):
        res = super()._get_asset_nodes(
            bundle, css=css, js=js, debug=debug, defer_load=defer_load, lazy_load=lazy_load, media=media, autoprefix=autoprefix,
        )
